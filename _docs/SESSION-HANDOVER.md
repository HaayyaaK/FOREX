...
6. **Repository follow-up** — `FOREX/` is now git-initialised with one commit; still needed: `_docs/CHANGELOG.md`, a remote (GitHub), and a decision on whether `C:\trading-proxy` becomes its own repo, a submodule, or stays separate.
